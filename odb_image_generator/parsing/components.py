...
def _detect_units_scale(text: str) -> float:
    """Return scale factor from first UNITS= line (25.4 for INCH, 1.0 for MM)."""
...
def parse_components_file(components_txt: str, side: Literal["TOP", "BOTTOM"]) -> List[Placement]:
    ...
    """
    scale = _detect_units_scale(components_txt)
    ...
    for raw in components_txt.splitlines():
        ...
        if line.startswith("CMP "):
            ...
            current_placement = Placement(
                refdes=parts[6],
                x_mm=float(parts[2]) * scale,
                y_mm=float(parts[3]) * scale,
