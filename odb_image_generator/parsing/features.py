...
def parse_layer_data(feature_txt: str, scale: float = 1.0) -> LayerData:
    """Parse complete layer data (symbols + features) from features file."""
    symbols = parse_symbol_defs(feature_txt, scale)
    features = list(iter_features(feature_txt, scale))
...
def iter_features(feature_txt: str, scale: float = 1.0) -> Generator[Tuple[str, Any], None, None]:
    ...
    for raw in feature_txt.splitlines():
        ...
        if token == "OB" and len(parts) >= 4:
            in_poly = True
            poly_pts = [(float(parts[1]) * scale, float(parts[2]) * scale)]
            poly_kind = parts[3]
            continue

        # Polygon segment: OS x y
        if token == "OS" and in_poly and len(parts) >= 3:
            poly_pts.append((float(parts[1]) * scale, float(parts[2]) * scale))
            continue
        ...
        if token == "P" and len(parts) >= 4:
            yield ("P", (float(parts[1]) * scale, float(parts[2]) * scale, int(parts[3])))

        # Line: L x1 y1 x2 y2 sym_id
        elif token == "L" and len(parts) >= 6:
            yield ("L", (float(parts[1]) * scale, float(parts[2]) * scale, 
                        float(parts[3]) * scale, float(parts[4]) * scale, int(parts[5])))

        # Arc: A x1 y1 x2 y2 xc yc sym_id
        elif token == "A" and len(parts) >= 8:
            yield ("A", (float(parts[1]) * scale, float(parts[2]) * scale,
                        float(parts[3]) * scale, float(parts[4]) * scale,
                        float(parts[5]) * scale, float(parts[6]) * scale, int(parts[7])))
